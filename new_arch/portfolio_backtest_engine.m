classdef portfolio_backtest_engine < handle
        ...
        function run(this)
            ...
            for k = 1:numel(commonDates)
                ...
                for i=1:n
                    ...
                    [ok, finalTargetQty] = this.set_target_qty_downsize(dt, sym, targetQty, px, spec, specMap);
                    if ok
                ...
                this.Master.Portfolio.apply_borrow_cost(closePx, specMap, this.TradingDays);

                % 4) Append equity curve
                ...
                if this.EnableSanityWarnings
                    if height(this.Master.Portfolio.EquityCurve) >= 2
                        eq1 = this.Master.Portfolio.EquityCurve.Equity(end);
                        eq0 = this.Master.Portfolio.EquityCurve.Equity(end-1);
                        if abs(eq1 - eq0) < 1e-9 && executedTrades > 0
                            % no-op (keep silent) - optimization loop prints its own warnings
                        end
                    end
                end
    ...
    methods (Access=private)
        function [ok, finalTargetQty] = set_target_qty_downsize(this, dt, sym, targetQty, px, spec, specMap)
            ...
            while iter <= this.DownsizeMaxIter
                ok = this.Master.Portfolio.set_target_qty(dt, sym, tq, px, spec, "", specMap);
                if ok
                    finalTargetQty = tq;
                    return;
                end
